...
int coordinate(int world_size) {
    ...
    CUFFT_CALL(cufftMakePlanMany64(planR2C, 2, n, nullptr, 0, 0, nullptr, 0, 0, cuFFT<double>::R2Ctype, static_cast<long long int>(Nx), &ws_r2c));
...
int compute(int rank, int distributor){
    ...
    MPI_Isend(send_ptr, transposed_dim.size_x[pidx_i]*Ny*transposed_dim.size_z[pidx_j]*sizeof(C_t), MPI_BYTE, distributor, distributor+1, MPI_COMM_WORLD, &send_req);
