...
namespace Difference_Slab_Z_Then_YX {
    ...
    __global__ void derivativeCoefficients(cuFFT<float>::C_t* out, int Nx, int Ny, int Nz, int Nz_offset, int N2){
        int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i < Nx*Ny*N2) {
            // get loop variables
            int z = i % N2;
            int y = (i-z)/N2 % Ny;
            int x = (i-z-y*N2) / (Ny*N2);
            ...
            if (y < Ny/2) k2 = y;
            else if (y > (int)(Ny/2)) k2 = Ny - y;

            if (z+Nz_offset < Nz/2) k3 = z+Nz_offset;

            double scale = -powf(k1, 2)-powf(k2, 2)-powf(k3, 2);

            out[x*Ny*N2+y*N2+z].x = static_cast<float>(static_cast<double>(out[x*Ny*N2+y*N2+z].x)*scale/sqrtf(Nx*Ny*Nz));
            out[x*Ny*N2+y*N2+z].y = static_cast<float>(static_cast<double>(out[x*Ny*N2+y*N2+z].y)*scale/sqrtf(Nx*Ny*Nz));
        }
    }

    __global__ void derivativeCoefficients(cuFFT<double>::C_t* out, int Nx, int Ny, int Nz, int Nz_offset, int N2){
        int i = blockIdx.x * blockDim.x + threadIdx.x;
        if (i < Nx*Ny*N2) {
            // get loop variables
            int z = i % N2;
            ...
            out[x*Ny*N2+y*N2+z].x = out[x*Ny*N2+y*N2+z].x*scale/sqrtf(Nx*Ny*Nz);
            out[x*Ny*N2+y*N2+z].y = out[x*Ny*N2+y*N2+z].y*scale/sqrtf(Nx*Ny*Nz);
        }
    }
...
int Tests_Slab_Random_Z_Then_YX<T>::testcase2(const int opt, const int runs){
    ...
    //execute
    this->initializeRandArray(out_d, Nx*Ny*N2);
    for (int i = 0; i < runs; i++){
        MPI_Barrier(MPI_COMM_WORLD);
...
template<typename T> 
int Tests_Slab_Random_Z_Then_YX<T>::testcase3(const int opt, const int runs){
    using R_t = typename cuFFT<T>::R_t;
    using C_t = typename cuFFT<T>::C_t;
    int provided;
    cublasHandle_t handle;
...
template<typename T> 
int Tests_Slab_Random_Z_Then_YX<T>::testcase4(const int opt, const int runs){
    using R_t = typename cuFFT<T>::R_t;
    using C_t = typename cuFFT<T>::C_t;
    int provided;
    cublasHandle_t handle;
    ...
    size_t Nz_offset = rank * (Nz/2+1) / world_size;
    if (rank < (Nz/2+1)%world_size)
        Nz_offset += rank;
    else 
        Nz_offset += (Nz/2+1)%world_size;
    ...
    for (int i = 0; i < runs; i++){
        ...
        Difference_Slab_Z_Then_YX::derivativeCoefficients<<<(Nx*Ny*N2)/1024+1, 1024>>>(out_d, Nx, Ny, Nz, Nz_offset, N2);
