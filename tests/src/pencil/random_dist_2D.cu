...
namespace Difference_Pencil_2D {
    // Difference
...
template<typename T> 
int Tests_Pencil_Random_2D<T>::run(const int testcase, const int opt, const int runs){
    if (testcase == 0)
...
template<typename T> 
int Tests_Pencil_Random_2D<T>::testcase0(const int opt, const int runs){
...
template<typename T> 
int Tests_Pencil_Random_2D<T>::coordinate(const int world_size, const int runs) {
    using R_t = typename cuFFT<T>::R_t;
    using C_t = typename cuFFT<T>::C_t;
    ...
    for (int i = 0; i < runs; i++) {
        ...
        for (size_t p_i = 0; p_i < P1; p_i++){
            for (size_t p_j = 0; p_j < P2; p_j++){
                ...
                MPI_Irecv(&recv_ptr[recv_count],  transposed_dim.size_z[p_j]*Ny*transposed_dim.size_x[p_i]*sizeof(C_t), 
                    MPI_BYTE, p_i*P2+p_j, world_size, MPI_COMM_WORLD, &recv_req[p_i*P2+p_j]);
                recv_count += transposed_dim.size_z[p_j]*Ny*transposed_dim.size_x[p_i];
        ...
        long long int n[2] = {static_cast<long long int>(Ny), static_cast<long long int>(Nz)};
    
        CUFFT_CALL(cufftMakePlanMany64(planR2C, 2, n, nullptr, 0, 0, nullptr, 0, 0, cuFFT<T>::R2Ctype, static_cast<long long int>(Nx), &ws_r2c));
        ...
        do {
            ...
            cpy_params.srcPtr = make_cudaPitchedPtr(&recv_ptr[recv_counts[p]], transposed_dim.size_z[p_j]*sizeof(C_t), transposed_dim.size_z[p_j], Ny);
        ...
        Difference_Pencil_2D::Difference<T>::difference<<<(Nx*Ny*(Nz/2+1))/1024+1, 1024>>>(complex, res_d, Nx*Ny*(Nz/2+1));
        CUDA_CALL(cudaDeviceSynchronize());
...
template<typename T> 
int Tests_Pencil_Random_2D<T>::compute(const int rank, const int world_size, const int opt, const int runs){
    using R_t = typename cuFFT<T>::R_t;
    using C_t = typename cuFFT<T>::C_t;
    ...
    if (config.cuda_aware){
        ...
        CUDA_CALL(cudaMallocHost((void **)&send_ptr, transposed_dim.size_x[pidx_i]*Ny*transposed_dim.size_z[pidx_j]*sizeof(C_t)));
    }
    ...
    for (int i = 0; i < runs; i++) {
        ...
        MPI_Isend(send_ptr, transposed_dim.size_x[pidx_i]*Ny*transposed_dim.size_z[pidx_j]*sizeof(C_t), MPI_BYTE, world_size, world_size+1, MPI_COMM_WORLD, &send_req);
...
template<typename T> 
int Tests_Pencil_Random_2D<T>::testcase2(const int opt, const int runs){
    return 0;
}

template<typename T> 
int Tests_Pencil_Random_2D<T>::testcase3(const int opt, const int runs){
    using R_t = typename cuFFT<T>::R_t;
    using C_t = typename cuFFT<T>::C_t;
    ...
    for (int i = 0; i < runs; i++) {
        ...
        mpicuFFT->execR2C(out_d, in_d, 2);
        MPI_Barrier(MPI_COMM_WORLD);
        mpicuFFT->execC2R(inv_d, out_d, 2);
        MPI_Barrier(MPI_COMM_WORLD);

        //compare difference
        Difference_Pencil_2D::Difference<T>::differenceInv<<<(input_dim.size_x[pidx_i]*input_dim.size_y[pidx_j]*Nz)/1024+1, 1024>>>(inv_d, in_d, input_dim.size_x[pidx_i]*input_dim.size_y[pidx_j]*Nz, Ny*Nz);
        T sum = 0;
...
template<typename T> 
int Tests_Pencil_Random_2D<T>::testcase4(const int opt, const int runs){
    return 0;
}

template class Tests_Pencil_Random_2D<float>;
template class Tests_Pencil_Random_2D<double>;
